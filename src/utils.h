...
template <class T>
vector<T> generate_data(distr_t data_distr, size_t size) {
  switch (data_distr) {
    case CHI_SQUARED:
    ...
    case NORMAL:
      return normal_distr<T>(size);
      break;

    case REVERSE_SORTED_UNIFORM:
      return reverse_sorted_uniform_distr<T>(size);
      break;

    case ROOT_DUPS:
      return root_dups_distr<T>(size);
      break;
